...
5. examTime（笔试时段）：笔试当天的具体时段（如"9:00-11:30"）。**重要负向约束**：咨询时间（"咨询时间：工作日上午9:00-11:30"）、报名时间（"报名时间…9:00-16:00"）、资格审核/复审时间、面试时间都不是笔试时间，禁止提取；整条免笔试的公告 examTime 必须为 null。
6. examType：只能从 [事业单位, 公务员, 教师招聘, 三支一扶, 医疗卫生, 国企招聘, 选调生, 公安辅警, 其他] 中选。警务辅助人员招聘填"公安辅警"。
7. 无把握的字段返回 null，禁止编造。但如果原文明确写了科目/日期，必须提取出来，不要因为格式不标准而漏掉。
只输出 JSON，不要输出任何其他文字。`;
