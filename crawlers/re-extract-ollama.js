...
6. examNote（笔试状态，三态）：判定规则——
   - 整条公告无笔试环节 → "免笔试"：标题含"博士/高级/高层次人员/岗位"专项（如"公开招聘博士人员公告""公开招聘高级及博士人员公告"）、或正文"面向博士/高级/高层次(的)招聘采取简化程序直接面试"、或"全部采取直接面试"、或"考核方式为试讲和答辩"
   - 部分岗位免笔试 → "部分岗位免笔试"："XX岗位(人员)可以/可采取简化程序直接面试"（如"副高级以上专业技术岗位人员，可以采取"）、"部分岗位采取简化程序"等
   - 原文出现"笔试内容/笔试时间/笔试科目/笔试地点/笔试成绩"等字样 → 有笔试环节，填 null
   - 原文无任何考试环节描述 → null（考试方式未知，不臆测）
7. 无把握的字段返回 null，禁止编造。但如果原文明确写了科目/日期/人数，必须提取出来，不要因为格式不标准而漏掉。

【输出前自检】逐项确认：
① 人数=各岗位之和了吗？② 多岗位的科目都分别列出来了吗？③ 报名截止取的是"至"后面的第二个日期吗？④ 免笔试是整条还是部分岗位（三态选对）？⑤ 有没有字段原文明明有却被漏掉？
只输出 JSON，不要输出任何其他文字。`;
...
/** HTML 剥离 + 归一化；16KB 上限覆盖 139 条全部长文本（实测最长剥离后 11KB），保证长公告后半段信息不丢失 */
function cleanHtml(raw, limit = 16000) {
  return (raw || '')
    .replace(/<[^>]+>/g, ' ')
    .replace(/\s+/g, ' ')
    .replace(/(\d)\s+(\d)/g, '$1$2')
 ...
 * 重试策略：第 1 次完整 16KB；后续重试截断到 8KB / 4KB（极端长文本超时时简化输入）
 */
